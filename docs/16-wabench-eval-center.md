...
进入 Task 12 shadow 的最小条件则较窄：共享 Rubric/硬失败/隐私/候选冻结协议可用、回滚开关保留，并且 V2 已在强制 JWT 的真实外层路径完成首写与同会话续写。真实 `local-pg-kb` 外层轨迹和受控异常降级/拒绝轨迹是 shadow 退出条件，不是可以伪造为 V1 Lexiang-only 结果的替代物。
