...
func (s *Store) CreateKnowledge(ctx context.Context, k EditorialKnowledge) (*EditorialKnowledge, error) {
	...
	err := s.db.QueryRowContext(ctx, `
		INSERT INTO editorial_knowledge (category, column_tag, title, content, source_task_id, source_artifact_id, confidence)
	...
	`,
		k.Category, k.ColumnTag, k.Title, k.Content, k.SourceTaskID, k.SourceArtifactID, k.Confidence,
	).Scan(
...
func (s *Store) ListKnowledge(ctx context.Context, category, columnTag string, limit int) ([]EditorialKnowledge, error) {
	...
	if columnTag != "" {
		query += fmt.Sprintf(" AND (column_tag = $%d OR column_tag = '')", argIdx)
