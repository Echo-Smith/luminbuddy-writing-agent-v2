...
// ── Email verification stubs (open-source version) ──

// EmailService is a stub for the commercial email verification service.
type EmailService struct{}

// NewEmailService creates a nil EmailService (stub).
// SMTPConfig is defined in config.go (shared file).
func NewEmailService(cfg interface{}) *EmailService {
	return nil
}

func (s *Server) handleSendVerificationCode(w http.ResponseWriter, r *http.Request) {
	response.Err(w, http.StatusServiceUnavailable, "email_disabled", "邮箱验证未启用")
}

func (s *Server) handleRegisterWithEmail(w http.ResponseWriter, r *http.Request) {
	response.Err(w, http.StatusServiceUnavailable, "email_disabled", "邮箱注册未启用")
}

func (s *Server) handleForgotPassword(w http.ResponseWriter, r *http.Request) {
	response.Err(w, http.StatusServiceUnavailable, "email_disabled", "密码重置未启用")
}
...
func (s *Server) handleBindEmail(w http.ResponseWriter, r *http.Request) {
	response.Err(w, http.StatusServiceUnavailable, "email_disabled", "邮箱绑定未启用")
}

func (s *Server) handleUnbindEmail(w http.ResponseWriter, r *http.Request) {
	response.Err(w, http.StatusServiceUnavailable, "email_disabled", "邮箱解绑未启用")
}

func (s *Server) handleGetEmail(w http.ResponseWriter, r *http.Request) {
	response.OK(w, map[string]interface{}{"email": "", "verified": false})
}
